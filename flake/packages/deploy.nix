{ bash, deno, elm-to-esm, elmPackages, git, writeScriptBin }:
    writeScriptBin "deploy" ''
      ...
      (cd "''${outdir}/server" && ${deno}/bin/deno run --allow-sys --allow-env --allow-read --allow-write=deno.json --allow-write=~/.deno --allow-net jsr:@deno/deployctl deploy --save-config --project "''${project}")
