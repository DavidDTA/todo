{ bash, elmPackages, jq, writeScriptBin }:
  writeScriptBin "quickfix" ''
    ...
    set -eEuo pipefail
    (cd elm && {
      ${elmPackages.lamdera}/bin/lamdera make --no-wire src/Frontend/Login.elm src/Frontend/Main.elm src/Backend/Main.elm --report=json --output=/dev/null >/dev/null || true
