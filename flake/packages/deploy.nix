{ bash, deno, elmPackages, git, writeScriptBin }:
    writeScriptBin "deploy" ''
      ...
      (cd elm && ${elmPackages.elm}/bin/elm make src/Login.elm --optimize --output=''${outdir}/server/index-unauthenticated.html)
      (cd elm && ${elmPackages.elm}/bin/elm make src/Frontend.elm --optimize --output=''${outdir}/server/index-authenticated.html)
